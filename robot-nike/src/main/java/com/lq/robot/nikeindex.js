/**
 * Created by liuqiang on 2018/5/30.
 */
(window.NREUM || (NREUM = {})).loader_config = {xpid: "VQYGVF5SCBAJVlFaAQIH"};
...
< script >
var nike = nike || {};
nike.exp = nike.exp || {}, nike.exp.script = nike.exp.script || {}, nike.exp.script.device_detect = nike.exp.script.device_detect || {}, function () {
    function e(e) {
        return function (e) {
                var i, t = document.cookie.split(";"), n = "", r = "", o = !1;
                for (i = 0; i < t.length; i++) {
                    if ((n = t[i].split("="))[0].replace(/^\s+|\s+$/g, "") == e)return o = !0, n.length > 1 && (r = unescape(n[1].replace(/^\s+|\s+$/g, ""))), r;
                    n = null
                }
                if (!o)return null
            }("sp") == e
    ...
    function i() {
        ...
        if (!/(^|&)ref=/.test(window.location.search)) {
            ...
            var t = window.location.hostname, n = t.split(".");
            if (e = e.indexOf("/", 8) > -1 ? e.slice(0, e.indexOf("/", 8)) : e, n.length > 1) {
                var r = n.slice(n.length - 2);
                t = r ? "." + r.join(".") : t
            }
            e && e.indexOf(t) > -1 && (e = "")
        }
        return e && e.length > 0 && (i && i.length > 0 ? i += "&ref=" + e : i = "?ref=" + e), i
    }

    function t(e) {
    }

    nike.exp.script.device_detect.redirectOnAgentMismatch = function (i, n) {
        if (void 0 !== i && n) {
            var r = ("" + i).toLowerCase() === ("" + nike.exp.script.device_detect.isMobile()).toLowerCase();
            if (t("isPresentationMobile=" + i + " isMobileAgent=" + nike.exp.script.device_detect.isMobile() + " presentationMatchesDevice=" + r), !e("dev") && !r && !e("mobile") || e("mobile") && "false" == i) {
                var o = nike.exp.script.device_detect.buildNewUrl((c = n, (a = document.createElement("textarea")).innerHTML = c, a.value));
                window.location != o && (window.location = o)
            }
        } else t("Presentation vars are not defined, so we are skipping the agent redirect check");
        var c, a
    }, nike.exp.script.device_detect.buildNewUrl = function (e) {
        var t = nike.exp.script.device_detect, n = "", r = e, o = {}, c = {};
        if (-1 != e.indexOf("?")) {
            var a = e.split("?");
            r = a[0];
            var s = a[1];
        ...
        return n = r + (p.length > 0 ? "?" + p.join("&") : ""), window.location.hash && (n += window.location.hash), n
    }, nike.exp.script.device_detect.buildParamMap = function (e) {
        var i = {}, t = [];
        if (0 == e.length)return {};
        t = -1 == e.indexOf("&") ? [e] : e.split("&");
        for (var n in t) {
            var r = t[n].split("="), o = r[0], c = "";
            2 == r.length && (c = r[1]), i[o] = c
        }
        return i
    }, nike.exp.script.device_detect.doAgentRedirect = function (n, r, o) {
        var c, a, s, d, l = window.location;
        void 0 === n || void 0 === r || void 0 === o ? t("Presentation vars are not defined, so we are skipping the agent redirect check") : (d = ("" + n).toLowerCase() === ("" + nike.exp.script.device_detect.isMobile()).toLowerCase(), t("isPresentationMobile=" + n + " isMobileAgent=" + nike.exp.script.device_detect.isMobile() + " presentationMatchesDevice=" + d), (!e("dev") && !d && !e("mobile") || e("mobile") && "false" == n) && ((a = "http:" === l.protocol.toLowerCase() ? r : o) && a.length > 0 ? (c = a + l.pathname, (s = i()) && s.length > 0 && (c += s), l.hash && (c += l.hash), window.location = c) : t("No alternate url defined.")))
    }, nike.exp.script.device_detect._isMobile = void 0, nike.exp.script.device_detect.isMobile = function () {
        return t("UserAgent: " + navigator.userAgent), nike.exp.script.device_detect._isMobile = /iPhone|iPod|Windows Phone|IEMobile/i.test(navigator.userAgent) || /android.*mobile/i.test(navigator.userAgent) || /mozilla.*mobile.*firefox/i.test(navigator.userAgent), nike.exp.script.device_detect._isMobile
    }
}();